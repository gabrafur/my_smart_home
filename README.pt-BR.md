...
| Flows com recovery | [`flows.json`](nodered/flows.json), [guia visual dos canvases](docs/node-red/CANVAS_STYLE_GUIDE.md), [replays](nodered/tools/test-security-recovery-flow.mjs), [casos adversariais](nodered/tools/test-security-recovery-adversarial.mjs) | restart, ordem de eventos e estado inválido falham de forma segura |
| Observabilidade | [monitoramento](docs/ZIGBEE_HEALTH_NOTIFICATIONS.md), [saúde das VPNs](docs/VPN_HEALTH_NOTIFICATIONS.md), [falhas globais do Node-RED](docs/NODERED_GLOBAL_FAILURE_NOTIFICATIONS.md), [saúde do host](docs/RASPBERRY_PI_SYSTEM_HEALTH.md), [testes de runtime](nodered/tools/test-infrastructure-monitoring-runtime.mjs) | falha, recuperação e deduplicação são estados testáveis, não só logs |
