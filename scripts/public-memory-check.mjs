...
export const instructionFiles = [
  `${instructionRoot}/codex-general.md`,
...
const instructionPreamble = `<!-- Generated by scripts/public-memory-check.mjs --generate-instructions. Do not edit directly. -->
...
const maxAgentInstructionBytes = 32 * 1024;
...
function isPublicInstructionFile(file) {
  return instructionFiles.includes(file);
}
...
export function renderAgentInstructions(repoRoot = defaultRepoRoot) {
  const modules = instructionFiles.map((file) => {
    const content = fs.readFileSync(path.join(repoRoot, file), "utf8").trim();
    if (!content) throw new Error(`instruction module is empty: ${file}`);
    return content;
  });
...
export function checkPublicMemory({ repoRoot = defaultRepoRoot, trackedFiles } = {}) {
  ...
  const instructionSources = ["AGENTS.md", ...instructionFiles].filter((file) => tracked.has(file));
  const publicMarkdownSources = [...instructionSources, ...memorySources];
  ...
  for (const source of publicMarkdownSources) {
    ...
    for (const { value, offset } of referencedCodeSpans(content)) {
      if (value.startsWith("make ")) {
        const commandTokens = value.slice(5).trim().split(/\s+/)
          .filter((token) => !token.startsWith("-") && !/^[A-Za-z_][A-Za-z0-9_]*=/.test(token));
        for (const target of commandTokens) {
  ...
  if (instructionFiles.every((file) => tracked.has(file))
    && agents !== renderAgentInstructions(repoRoot)) {
    errors.push("AGENTS.md: generated instruction aggregate is stale");
  }
  const instructionBundle = instructionFiles.filter((file) => tracked.has(file)).map(readTracked).join("\n");
  for (const skill of [...publicAgentFiles].filter((file) => file.endsWith("/SKILL.md"))) {
    if (!instructionBundle.includes(skill)) {
      errors.push(`Codex instructions: canonical repository skill is not declared: ${skill}`);
    }
  }
  let previousAuthorityOffset = -1;
  for (const marker of authorityMarkers) {
    const offset = instructionBundle.indexOf(marker);
    if (offset === -1 || offset <= previousAuthorityOffset) {
      errors.push(`Codex instructions: missing or unordered memory authority rule: ${marker}`);
      break;
    }
    previousAuthorityOffset = offset;
  }
  if (!instructionBundle.includes(".codex/memories/<assunto>/<nome-descritivo>.md")) {
    errors.push(`Codex instructions: canonical memory location is not declared`);
  }
  ...
  for (const source of instructionSources) {
    errors.push(...uniqueHeadingErrors(source, readTracked(source)));
  }
  ...
  if (!weeklyPrompt.includes(".codex/instructions/**")) {
    errors.push("scripts/weekly-docs-review.prompt.md: canonical instruction glob is missing");
  }
  ...
  return {
    ...
    stats: {
      ...
      linksChecked: publicMarkdownSources.reduce(
        (total, source) => total + markdownLinks(readTracked(source)).length,
        0,
      ),
    },
  };
...
function main() {
  if (process.argv[2] === "--generate-instructions" && process.argv.length === 3) {
    fs.writeFileSync(path.join(defaultRepoRoot, "AGENTS.md"), renderAgentInstructions(defaultRepoRoot));
    console.log(`Generated AGENTS.md from ${instructionFiles.length} modules.`);
    return;
  }
  if (process.argv.length > 2) {
    console.error("usage: public-memory-check.mjs [--generate-instructions]");
    process.exit(1);
  }
  ...
  console.log(
    `Public memory check passed: ${result.stats.instructionFiles} instruction files, `
      + `${result.stats.memoryFiles} memory files, `
