...
| Retry/cooldown Bluelink | `creta_refresh_decide`/ack | button refresh | `security_creta_refresh_v1` persistent | intenção temporal | não físico; temporal | persistir | storm ou retry nunca retomado | backoff 1/2/4/8/15 min; o contador satura em cinco e novas tentativas permanecem espaçadas em 15 min; sucesso zera e arma 15 min; deadline absurdo é descartado |
...
| Caches people/Creta do coordenador | contratos recebidos | política conjunta/aviso | flow memory | snapshot derivado | derivável; transitório | reconstruir | antigo sobrescrever novo | aceitar apenas `updated_at` monotônico; futuro >60 s é rejeitado; conflito no mesmo timestamp preserva o primeiro; snapshot posterior `ready: false` prevalece para bloquear efeitos |
| Candidato/aviso da Valéria | people → coordinator → notify | usuário | `security_pending_valeria_notification_v1` persistent até emissão | evento externo/side effect pendente | temporal; intenção de entrega | persistir no máximo 10 min | notificação perdida ou duplicada durante startup | candidato aguarda Creta ready, é enriquecido com o snapshot atual, removido antes da emissão e expira sem side effect |
...
| `light_reconciled` / `security_light_ready` | reconciliador | preparação de chegada/recovery | flow memory | readiness derivada | derivável; transitório | reconstruir | side effect durante startup parcial | true apenas com pessoas, Creta, sol e switch físico prontos, incluindo observação física nos últimos 2 min |
