...
| Chegada pendente da iluminação | evento canônico de chegada de morador após gate visual de direção | merge de contexto e gate de acendimento | `security_light_pending_arrival_v1` e `security_light_local_excursion_v1` persistent | intenção de replay | intenção; temporal | persistir por até 15 min; passeio local por 90 min | perder o acendimento enquanto Bluelink ou sol converge | o caminho externo exige retorno confirmado e localização atual em `near_home`; o caminho local nunca usa o primeiro `on`, exige `home -> near_home`, um `off` posterior e novo `on`, e aceita localização atual em `near_home` ou `home`; ambos voltam pelos mesmos gates de noite, motor/bypass, disponibilidade, dedupe e cooldown; o caminho local não alcança avisos nem alarme |
