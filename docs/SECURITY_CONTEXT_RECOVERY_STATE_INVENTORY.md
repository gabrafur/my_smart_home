...
| Candidato/aviso da resident_secondary | people → coordinator → notify | usuário | `security_pending_resident_secondary_notification_v1` persistent até emissão | evento externo/side effect pendente | temporal; intenção de entrega | persistir no máximo 10 min | notificação perdida ou duplicada durante startup | candidato aguarda vehicle_primary ready, é enriquecido com o snapshot atual, removido antes da emissão e expira sem side effect |
