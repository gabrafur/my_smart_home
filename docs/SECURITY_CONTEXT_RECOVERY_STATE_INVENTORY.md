...
| Chegada pendente da iluminação | evento canônico de chegada de morador após gate visual de direção | merge de contexto e gate de acendimento | `security_light_pending_arrival_v1` persistent | intenção de replay | intenção; temporal | persistir por até 10 min | perder o acendimento enquanto Bluelink ou sol converge | persiste apenas o contrato reexecutável, nunca o objeto transitório da função; exige morador vindo de `not_home`/zona externa para `near_home` ou recovery de ciclo externo já armado, `arrival_direction: returning`, ciclo externo confirmado e localização atual; posição/chegada do veículo e estágio `home` nunca autorizam acendimento; replay volta pelo mesmo gate; evento confirmado de motor `on` ou ativação válida do bypass também reavalia imediatamente o morador que permaneça armado em `near_home`, mesmo após expirar a intenção temporária; cancela ao entrar em casa, sair da zona, ficar stale ou expirar |
...
| `vehicle_refresh_at`, reason e source | confirmação `home` de morador com refletor ativo | recovery e atualização extraordinária do vehicle_primary | lifecycle persistent + delay variável | confirmação temporal | intenção; temporal | persistir | manter o ciclo normal de 30 min e atrasar o desligamento | deadline 90 s desde a confirmação `home` do morador, nunca desde `near_home`, desde o acendimento ou desde a posição `home` do veículo; pode ser agendado mesmo com a integração do carro indisponível; ao vencer exige leitura física recente, revalida `home` da origem e força refresh pelo coordenador; a confirmação não desliga diretamente e cancela se a condição sumiu |
