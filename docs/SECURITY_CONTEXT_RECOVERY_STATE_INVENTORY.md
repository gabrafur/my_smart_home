...
| Reserva e dedupe dos avisos recíprocos | `notificacoes_chegadas_residentes` | notify do outro residente | persistent em `resident_notification_delivery_v2`; testes somente em memória | recibo e idempotência da entrega | parcialmente derivável; temporal | persistir recibo; reserva expira | aviso duplicado após restart ou retry concorrente | contrato de chegada já traz o ciclo externo; chave origem/estágio/timestamp tem TTL visual de 10 min; reserva precede o serviço, falha a libera e aceite confirma o recibo; o estado legado v1 permanece intocado apenas para rollback |
...
| Avisos de chegada entre residentes | `security.arrival.v1` do lifecycle canônico → reserva/dedupe → notify | usuário | `resident_notification_delivery_v2` | evento externo/side effect | temporal | persistir recibo e tentativas | notificação perdida na chegada direta em casa ou duplicada após restart | aceita `approach` e `home`, exige `returning` e ciclo externo confirmado; emissão independente dos demais domínios; nenhum tracker bruto alimenta este tab |
