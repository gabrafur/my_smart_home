...
- `primary_home`, `any_tracker_home` e tempo do tracker primário em casa;
...
- `in_use` (`creta_in_use`) e armado de chegada.
...
só publica `security.refresh-command.v1` quando ambos retornam no mesmo ciclo.
Isso evita duplicação com dois eventos quase simultâneos. Cada domínio continua
dono de seu cooldown.
...
Quando ambos os trackers têm coordenadas confiáveis, conserva-se o fallback
anterior: vence o tracker que reporta a maior distância de casa, porque a falha
...
- só desliga com motor `off` e (porta destravada ou carro em casa).
...
4. o refletor não foi marcado como ativo por chegada;
5. não há supressão pós-desligamento ativa.

Ao ligar, grava em flow context:

- `refletor_portao_carros_active_by_arrival = true`;
- `refletor_activated_at = Date.now()`.
...
Uma atualização genérica da trava não ignora o filtro de 5 s. A carência lê
`msg.delay`, portanto desconta o tempo que já passou desde o acendimento. Após
qualquer desligamento, há cinco minutos de supressão contra religamento.
...
- O timestamp do Creta só é gravado após o `button.press` retornar com sucesso;
  uma falha permite retry no ciclo seguinte.
...
`nodered/settings.js` não configura `contextStorage: localfilesystem`; logo o
flow context padrão é memória. A refatoração não muda esse comportamento.

| Estado | Dono | Após restart completo do Node-RED |
| --- | --- | --- |
| armado de Gabriel/Valéria | `localizacao_pessoas` | reconstruído pelo snapshot; janela curta sem histórico de travessia |
| armado e `creta_in_use` | `contexto_creta` | reconstruído do snapshot; `creta_in_use` pode ficar falso durante uma viagem se o motor já estiver stale `off` |
| cooldowns de refresh | cada domínio | perdidos; o primeiro ciclo pode atualizar novamente |
| dedup do aviso da Valéria | `localizacao_pessoas` | perdido; uma nova transição pode avisar novamente |
| ativo por chegada, ativação e supressão | `iluminacao_seguranca` | perdidos; o switch físico não é reconciliado automaticamente |
| delays de 90 s e 15 min | runtime Node-RED | cancelados |

No restart do Home Assistant, as chamadas que falharem não confirmam o
cooldown do Creta. O tick volta a pedir snapshots. A luminosidade usa
`outputInitially: true`, restaurando o contexto solar assim que a conexão HA
fica disponível.

Persistir/reconciliar esses estados seria uma melhoria funcional e ficou fora
desta refatoração. Até isso ser implementado, um restart durante viagem ou com
o refletor ligado exige atenção operacional.
...
de refresh. É um replay offline dos `function nodes` e uma validação estrutural;
