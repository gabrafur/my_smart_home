...
keeps only six automations that belong to the platform boundary or provide a
...
| `portao_garagem_carimba_pulso_externo` | stamp every relay closure into the shared cooldown | bridges Node-RED pulses and Home Assistant commands without emitting `ON` |
...
None of the six automations was disabled or duplicated. Rolling back this
decision is documentation-only: any future migration must first create and
test the replacement flow while disabled, then perform one reversible cutover;
both paths must never remain able to execute the same action.
