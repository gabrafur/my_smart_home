...
na central do Home Assistant e envia a mesma mensagem somente ao papel lógico
`resident_primary` pelo hub móvel. Em paralelo, chama o hub Alexa com
`voice_assistant_primary`. Os hubs preservam `queue: all` para aguardar a
reconexão com o Home Assistant sem duplicar a decisão de resfriamento.
