...
assert.deepEqual(node("recorder_retention_dispatch_guard").wires, [["recorder_retention_rate_limit"], ["recorder_retention_dry_run_terminal"]]);
...
assert.deepEqual(cold.payload.map((entry) => entry.key), ["codex_diagnostics"]);
const warm = plan({ testNow: NOW + 2 * 24 * 60 * 60 * 1000 }, flow, runtimeNode(), {}, {});
assert.equal(warm.payload.length, 6);
assert.equal(warm.payload.find((entry) => entry.key === "raspberry_pi_health").keep_days, 2);
const testDispatch = guard({ test_mode: true, payload: warm.payload[0] }, flow, runtimeNode(), {}, {});
...
const productionDispatch = guard({ payload: warm.payload[1] }, flow, runtimeNode(), {}, {});
