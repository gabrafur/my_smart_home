...
upsert({ id: "gar_effect_pulse_in", type: "link in", z: TAB, g: "gar_group_effect", name: "Receber pulso decidido", links: ["gar_request_pulse_out"], x: 960, y: 450, wires: [["gar_pulse_test_gate"]] });
upsert({ id: "gar_effect_safe_off_in", type: "link in", z: TAB, g: "gar_group_effect", name: "Receber OFF seguro", links: ["gar_request_safe_off_out"], x: 960, y: 570, wires: [["gar_safe_off_test_gate"]] });
upsert({ id: "gar_pulse_test_gate", type: "switch", z: TAB, g: "gar_group_effect", name: "Gate final: pulso produção ou TESTE?", property: "test_mode", propertyType: "msg", rules: [{ t: "neq", v: "true", vt: "bool" }, { t: "eq", v: "true", vt: "bool" }], checkall: "true", repair: false, outputs: 2, x: 1160, y: 450, wires: [["gar_prepare_pulse_delay"], ["gar_effect_dry_run_out"]] });
upsert({ id: "gar_prepare_pulse_delay", type: "change", z: TAB, g: "gar_group_effect", name: "Aplicar largura da política", rules: [{ t: "set", p: "delay", pt: "msg", to: "policy.pulse_ms", tot: "msg" }], x: 1420, y: 420, wires: [["gar_relay_pulse_on", "gar_log_pulse_started"]] });
upsert(functionNode("gar_relay_pulse_on", "gar_group_effect", "Adaptar MQTT: fechar contato (ON)", mqttCommand("ON"), 1, 1700, 420, [["gar_relay_mqtt_out", "gar_relay_safety_delay"]]));
upsert({ id: "gar_relay_safety_delay", type: "delay", z: TAB, g: "gar_group_effect", name: "Aguardar msg.delay da política", pauseType: "delayv", timeout: "700", timeoutUnits: "milliseconds", rate: "1", nbRateUnits: "1", rateUnits: "second", randomFirst: "1", randomLast: "5", randomUnits: "seconds", drop: false, allowrate: false, outputs: 1, x: 1700, y: 480, wires: [["gar_relay_pulse_off"]] });
upsert(functionNode("gar_relay_pulse_off", "gar_group_effect", "Adaptar MQTT: abrir contato (OFF)", mqttCommand("OFF"), 1, 1960, 480, [["gar_relay_mqtt_out"]]));
upsert({ id: "gar_relay_mqtt_out", type: "mqtt out", z: TAB, g: "gar_group_effect", name: "Efeito: publicar comando do relé", topic: "", qos: "", retain: "", respTopic: "", contentType: "", userProps: "", correl: "", expiry: "", broker: BROKER, x: 1960, y: 420, wires: [] });
upsert({ id: "gar_log_pulse_started", type: "api-call-service", z: TAB, g: "gar_group_effect", name: "Confirmar pedido aceito no Logbook", server: HA_SERVER, version: 7, debugenabled: false, action: "logbook.log", floorId: [], areaId: [], deviceId: [], entityId: [], labelId: [], data: '{"name":"Portão garagem","message":"pulso ÚNICO iniciado (origem: " & request.origin & ") — largura " & policy.pulse_ms & " ms, cooldown " & policy.cooldown_ms & " ms."}', dataType: "jsonata", mergeContext: "", mustacheAltTags: false, outputProperties: [], queue: "none", blockInputOverrides: true, domain: "logbook", service: "log", x: 1700, y: 540, wires: [[]] });
upsert({ id: "gar_safe_off_test_gate", type: "switch", z: TAB, g: "gar_group_effect", name: "Gate final: OFF seguro ou TESTE?", property: "test_mode", propertyType: "msg", rules: [{ t: "neq", v: "true", vt: "bool" }, { t: "eq", v: "true", vt: "bool" }], checkall: "true", repair: false, outputs: 2, x: 1160, y: 570, wires: [["gar_effect_safe_off_adapter_out", "gar_notify_relay_on"], ["gar_effect_dry_run_out"]] });
upsert({ id: "gar_effect_safe_off_adapter_out", type: "link out", z: TAB, g: "gar_group_effect", name: "OFF seguro → adapter MQTT", mode: "link", links: ["gar_effect_safe_off_adapter_in"], x: 1420, y: 550, wires: [] });
upsert({ id: "gar_effect_safe_off_adapter_in", type: "link in", z: TAB, g: "gar_group_effect", name: "Receber OFF seguro", links: ["gar_effect_safe_off_adapter_out"], x: 1800, y: 600, wires: [["gar_relay_pulse_off"]] });
upsert({ id: "gar_effect_dry_run_out", type: "link out", z: TAB, g: "gar_group_effect", name: "TESTE → terminal dry-run", mode: "link", links: ["gar_test_dry_run_in"], x: 1420, y: 630, wires: [] });
upsert({ id: "gar_notify_relay_on", type: "api-call-service", z: TAB, g: "gar_group_effect", name: "Avisar: relé já estava ON", server: HA_SERVER, version: 7, debugenabled: false, action: "persistent_notification.create", floorId: [], areaId: [], deviceId: [], entityId: [], labelId: [], data: '{"notification_id":"portao_garagem_rele_preso","title":"Portão da garagem - relé estava ligado","message":"O relé já estava ON. O Node-RED enviou somente OFF e recusou um novo pulso."}', dataType: "jsonata", mergeContext: "", mustacheAltTags: false, outputProperties: [], queue: "all", blockInputOverrides: true, domain: "persistent_notification", service: "create", x: 1640, y: 600, wires: [[]] });
upsert(group("gar_group_effect", "3. Gate final, efeitos e confirmação", ["gar_effect_pulse_in", "gar_effect_safe_off_in", "gar_pulse_test_gate", "gar_prepare_pulse_delay", "gar_relay_pulse_on", "gar_relay_safety_delay", "gar_relay_pulse_off", "gar_relay_mqtt_out", "gar_log_pulse_started", "gar_safe_off_test_gate", "gar_effect_safe_off_adapter_out", "gar_effect_safe_off_adapter_in", "gar_effect_dry_run_out", "gar_notify_relay_on"], 920, 390, 1200, 270, "#dc2626", "#fee2e2"));
...
upsert({ id: "gar_test_instructions", type: "comment", z: TAB, g: "gar_group_tests", name: "Ordem: reset → aceito → duplicado → cooldown → limite → relé ON. Tudo termina em dry-run.", info: "Nenhum botão de teste alcança MQTT, Home Assistant, Logbook, notificação ou dispositivo.", x: 455, y: 700, wires: [] });
upsert(inject("gar_test_reset", "gar_group_tests", "TESTE 1: reset", "", "", "date", 180, 760, [["gar_test_reset_state"]]));
upsert(functionNode("gar_test_reset_state", "gar_group_tests", "Resetar estado sintético", testReset, 0, 430, 760, []));
upsert(inject("gar_test_accept", "gar_group_tests", "TESTE 2: pulso aceito", "", '{"action":"single","origem":"teste","test_mode":true,"_test_now_ms":2000000}', "json", 200, 820, [["gar_test_request_out_left"]]));
upsert(inject("gar_test_duplicate", "gar_group_tests", "TESTE 3: duplicado +500 ms", "", '{"action":"single","origem":"teste","test_mode":true,"_test_now_ms":2000500}', "json", 230, 870, [["gar_test_request_out_left"]]));
upsert(inject("gar_test_cooldown", "gar_group_tests", "TESTE 4: cooldown +950 ms", "", '{"action":"single","origem":"teste","test_mode":true,"_test_now_ms":2000950}', "json", 230, 920, [["gar_test_request_out_left"]]));
upsert(inject("gar_test_boundary", "gar_group_tests", "TESTE 5: limite +1.000 ms", "", '{"action":"single","origem":"teste","test_mode":true,"_test_now_ms":2001000}', "json", 235, 970, [["gar_test_request_out_left"]]));
upsert({ id: "gar_test_request_out_left", type: "link out", z: TAB, g: "gar_group_tests", name: "Cenários TESTE → caminho real", mode: "link", links: ["gar_request_test_in"], x: 510, y: 900, wires: [] });
upsert(inject("gar_test_relay_on", "gar_group_tests", "TESTE 6: relé já ON", "", '{"action":"single","origem":"teste","test_mode":true,"_test_now_ms":2003000}', "json", 700, 820, [["gar_test_relay_on_prepare"]]));
upsert(functionNode("gar_test_relay_on_prepare", "gar_group_tests", "Preparar relé sintético ON", testRelayOn, 1, 970, 820, [["gar_test_request_out_right"]]));
upsert({ id: "gar_test_request_out_right", type: "link out", z: TAB, g: "gar_group_tests", name: "Relé ON TESTE → caminho real", mode: "link", links: ["gar_request_test_in"], x: 1180, y: 820, wires: [] });
upsert({ id: "gar_test_dry_run_in", type: "link in", z: TAB, g: "gar_group_tests", name: "Receber efeito TESTE", links: ["gar_effect_dry_run_out"], x: 1240, y: 900, wires: [["gar_test_dry_run_terminal"]] });
