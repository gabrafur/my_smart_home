...
const groups = {
  ...
  test: group("arrival_context_test_group", "5. Teste coordenado — efeitos continuam até dry-run dos consumidores", 64, 1300, 2500, 300, "#0891b2", "#cffafe"),
};
...
grouped(groups.input, { id: "arrival_context_policy_note", type: "comment", z: TAB, g: groups.input,
  name: "Padrões: ciclo 30 s; em voo 10 s; tolerância futura 60 s.",
  info: "Limites: em voo 1–60 s e tolerância futura 0–300 s. Candidato inválido não substitui a última política válida.", x: 530, y: 60, wires: [] });
...
linkIn("arrival_context_cycle_input_in", groups.cycle, "Receber agenda, recovery ou TESTE", ["arrival_context_periodic_request_out", "arrival_context_recovery_request_out", "arrival_context_cycle_input_out", "arrival_context_test_request_out"], "arrival_context_cycle_policy_load", 1250, 310);
...
linkIn("arrival_context_snapshot_in", groups.snapshot, "Receber pessoas ou vehicle_primary", "arrival_context_snapshot_out", "arrival_context_snapshot_policy_load", 3890, 330);
...
linkOut("arrival_context_departure_out", groups.snapshot, "Snapshot aceito → saída", "arrival_context_departure_in", 7030, 580);
...
grouped(groups.test, { id: "arrival_context_test_note", type: "comment", z: TAB, g: groups.test,
  name: "Ordem: configure pessoas/vehicle_primary nos produtores → reset → ciclo recovery.",
  info: "Snapshots sintéticos retornam pelos mesmos links. Comandos seguem aos consumidores, cujos gates finais registram simulated:true e dispatched:false.", x: 820, y: 1340, wires: [] });
inject("arrival_context_test_reset_control", groups.test, "TESTE 1: reset coordenado", [{ p: "payload", v: JSON.stringify({ kind: "test_reset", test_mode: true, test_case: "context_coordinator" }), vt: "json" }, { p: "_location_test", v: "true", vt: "bool" }], 230, 1440, [["arrival_context_test_request_out"]]);
inject("arrival_context_test_cycle_control", groups.test, "TESTE 2: ciclo recovery", [{ p: "payload", v: JSON.stringify({ kind: "refresh_tick", reason: "manual_context_recovery", force_recovery: true, require_lighting_ready: true, test_mode: true, test_case: "context_coordinator" }), vt: "json" }, { p: "_location_test", v: "true", vt: "bool" }], 230, 1510, [["arrival_context_test_request_out"]]);
linkOut("arrival_context_test_request_out", groups.test, "Pedido TESTE → ciclo canônico", "arrival_context_cycle_input_in", 520, 1480);
