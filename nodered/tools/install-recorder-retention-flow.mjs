...
const nodes = [
  ...
  { id: "recorder_retention_group_execution", type: "group", z: TAB, name: "2. Alvos visíveis, purge serial e repack após a fila", style: { label: true, color: "#4d9a6a" }, nodes: ["recorder_retention_schedule", "recorder_retention_plan_targets", "recorder_retention_plan", "recorder_retention_split", "recorder_retention_dispatch_guard", "recorder_retention_rate_limit", "recorder_retention_purge", "recorder_retention_repack_delay", "recorder_retention_repack_status", "recorder_retention_repack_evaluate", "recorder_retention_repack", "recorder_retention_repack_retry_out", "recorder_retention_repack_delay_in", "recorder_retention_repack_test_in", "recorder_retention_guard_dry_out", "recorder_retention_repack_dry_out"], x: 64, y: 579, w: 1820, h: 202 },
  ...
  { id: "recorder_retention_schedule", type: "inject", z: TAB, g: "recorder_retention_group_execution", name: "Agenda visível — diariamente 04:30", props: [{ p: "payload" }], repeat: "", crontab: "30 04 * * *", once: false, onceDelay: "0.1", topic: "", payload: "", payloadType: "date", x: 220, y: 660, wires: [["recorder_retention_plan_targets"]] },
  { id: "recorder_retention_plan_targets", type: "change", z: TAB, g: "recorder_retention_group_execution", name: "Alvos canônicos de retenção", rules: [{ t: "set", p: "purgeTargets", pt: "msg", to: JSON.stringify(purgeTargets), tot: "json" }], x: 500, y: 660, wires: [["recorder_retention_plan"]] },
  functionNode("recorder_retention_plan", "recorder_retention_group_execution", "Calcular aquecimento e plano", planPurge, 2, 760, 660, [["recorder_retention_split"], ["recorder_retention_repack_delay"]]),
  { id: "recorder_retention_split", type: "split", z: TAB, g: "recorder_retention_group_execution", name: "Uma política por vez", splt: "\\n", spltType: "str", arraySplt: 1, arraySpltType: "len", stream: false, addname: "", property: "payload", x: 930, y: 620, wires: [["recorder_retention_dispatch_guard"]] },
  functionNode("recorder_retention_dispatch_guard", "recorder_retention_group_execution", "Gate de dispatch / dry-run", dispatchGuard, 2, 1180, 620, [["recorder_retention_rate_limit"], ["recorder_retention_guard_dry_out"]]),
  { id: "recorder_retention_rate_limit", type: "delay", z: TAB, g: "recorder_retention_group_execution", name: "Serializar: 1 purge a cada 2 min", pauseType: "rate", timeout: "5", timeoutUnits: "seconds", rate: "1", nbRateUnits: "2", rateUnits: "minute", randomFirst: "1", randomLast: "5", randomUnits: "seconds", drop: false, allowrate: false, outputs: 1, x: 1450, y: 620, wires: [["recorder_retention_purge"]] },
  { id: "recorder_retention_purge", type: "api-call-service", z: TAB, g: "recorder_retention_group_execution", name: "EFEITO: purge seletivo do Recorder", server: SERVER, version: 7, debugenabled: false, action: "recorder.purge_entities", floorId: [], areaId: [], deviceId: [], entityId: [], labelId: [], data: "payload", dataType: "jsonata", mergeContext: "", mustacheAltTags: false, outputProperties: [], queue: "none", blockInputOverrides: true, domain: "recorder", service: "purge_entities", x: 1750, y: 620, wires: [[]] },
