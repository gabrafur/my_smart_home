...
const groups = {
  ...
  health: group("local_ai_rtx_health_group", "1. Saúde, energia do host, decisões e estado", 800, 20, 4300, 590, "#0f766e", "#ccfbf1"),
  recovery: group("local_ai_rtx_recovery_group", "2. Gate final e recovery seguro via MCP", 800, 640, 2000, 280, "#dc2626", "#fee2e2"),
  test: group("local_ai_rtx_test_group", "3. Testes manuais completos — dry-run", 64, 960, 2736, 330, "#0891b2", "#cffafe"),
};
...
grouped(groups.policy, {
  ...
  info: "O Node-RED não executa WSL, Ollama, netsh, iphlpsvc nem SSH. A fonte sensor.codex_rtx_host_reachability_raw apenas confirma se o computador está ligado. Computador desligado silencia alertas; ligado com endpoint indisponível alerta uma vez por incidente.",
  x: 410, y: 60, wires: [],
});
...
change("local_ai_rtx_prepare_prod_alert_reset", groups.health, "Resetar incidente de produção", prepareResetRules("production"), 3500, 360, [["local_ai_rtx_alert_dedupe_reset_out"], ["local_ai_rtx_status_gate_out"]]);
...
linkOut("local_ai_rtx_alert_dedupe_reset_out", groups.health, "Reset → dedupe por modo", "local_ai_rtx_alert_dedupe_reset_in", 3800, 390);
...
rbe("local_ai_rtx_alert_rbe", groups.health, "Uma notificação por incidente e modo", "rtx_alert_condition", 4350, 500, [["local_ai_rtx_alert_build"]]);
...
fn("local_ai_rtx_alert_build", groups.health, "Adaptar condição para alerta de domínio", "local-ai-rtx-alert-build.js", 1, 4650, 500, [["local_ai_rtx_alert_out"]]);
linkOut("local_ai_rtx_recovery_out", groups.health, "Pedido permitido → gate final MCP", "local_ai_rtx_recovery_in", 3220, 290);
linkOut("local_ai_rtx_alert_out", groups.health, "Host ligado + RTX indisponível → entrega central", "global_observer_alert_to_dispatch_in", 4950, 500);
...
grouped(groups.test, {
  ...
  info: "Estados sintéticos usam contexto e dedupe separados. PC desligado silencia; PC ligado com listener ausente percorre a entrega central até o dry-run.",
  x: 900, y: 1000, wires: [],
});
