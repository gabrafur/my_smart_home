...
add({
  ...
  disabled: false,
  info: "Prioriza a saúde do servidor sem dar privilégios ao Node-RED. A cada minuto solicita ao worker do host uma avaliação fail-closed. Somente uma árvore antiga e desconectada do VS Code, duplicada por uma sessão mais nova conectada, pode ser encerrada após pressão de memória e ociosidade confirmadas.",
  env: [],
});
...
grouped(groups.request, {
  ...
  info: "O worker roda como usuário comum. Allowlist fechada: apenas árvores antigas de extensionHost do VS Code; sessão nova conectada, memória abaixo de 1.536 MiB/20%, idade mínima, RSS mínimo e dois ciclos ociosos são obrigatórios.",
  x: 720,
  y: 60,
  wires: [],
});
...
inject(
  ...
  resultProps({ status: "candidate_observed", available_mib: 1200, available_percent: 14.6, candidate_pid: "synthetic", candidate_mib: 640, terminated: 0, test_mode: true }),
  205,
  990,
  [["host_memory_guardian_test_result_out"]],
);
inject(
  ...
  resultProps({ status: "terminated", available_mib: 1200, available_percent: 14.6, candidate_pid: "synthetic", candidate_mib: 640, terminated: 4, test_mode: true }),
  215,
  1050,
  [["host_memory_guardian_test_result_out"]],
);
